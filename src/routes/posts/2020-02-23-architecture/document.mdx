...
[Grady Booch](https://www.youtube.com/watch?v=z0HsPBKfhoI), the IBM research scientist, observed “all architecture is design but not all design is architecture“. In other words, architecture does not happen by accident but requires a deliberate, thoughtful design process.
...
Architecture is not important for what is, but for what it does. Architecture is 'purpose driven', produced to achieve a specific outcome. At Strategic Machines, we like to think of an architecture's most fundamental purpose as not just solving the problem at hand but solving it in a manner that makes the complex simple. We're not talking about being simplistic, but simple. 
